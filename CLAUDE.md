...
Claude Code가 이 프로젝트에서 작업할 때 반드시 따라야 할 전체 규칙과 아키텍처 가이드입니다.
...
10. **코드 수정 시 문서 동시 업데이트 필수** — 아래 기준에 따라 해당 문서 업데이트:
   ...
   | 신기능 추가 | `MEMORY.md` + 아키텍처 변경 시 `CLAUDE.md` + 관련 스킬 파일 |
   | 아키텍처 변경 | `CLAUDE.md` (핵심 아키텍처 패턴 또는 DB 스키마 섹션) |
   | 절대 규칙 위반 패턴 발견 | `CLAUDE.md` 절대 규칙 섹션에 추가 |
...
1. **코드 위치·호출관계·영향범위는 파일을 읽기 전에 그래프에 먼저 질의** — `graphify` MCP 등록됨.
   `graphify query "<질문>"` / `affected "<심볼>"` / `god-nodes`. 코드 수정 후 `graphify update .`
2. **`app.js`(5,710줄)·`api.py`(5,501줄) 통째로 읽지 말 것** — grep으로 줄 번호를 특정한 뒤 범위만 읽기.
3. **한 세션 = 한 작업 단위** — 작업이 끝나면 세션을 종료할 것. 비용의 54%가 매 턴 컨텍스트
   재전송(cache_read)이라, 긴 세션은 컨텍스트 크기 × 턴 수로 비용이 복리로 늘어남.
   (실측: 3,267턴 세션 1개에서 cache_read 314M 토큰)
4. **규칙 문서는 이 파일 한 곳만** — `AGENTS.md`는 포인터. 양쪽에 같은 내용을 두지 말 것.
5. **코드를 쓰기 전 `ponytail` 스킬의 사다리를 적용** — `.claude/skills/ponytail/SKILL.md`.
   존재 필요성 → 기존 코드 재사용 → stdlib → 네이티브 기능 → 기존 의존성 → 한 줄 → 최소 구현.
