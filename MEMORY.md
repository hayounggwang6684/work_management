...
예: SH-2026-136-T 가 영광해운/영광호로 등록돼 있는데 같은 번호로 다른 작업 입력.

- 원인: `autoExpandContractNumber()` 는 **빈 필드만** 자동완성하고 이미 값이 든
  필드는 조용히 건너뛴다. 그래서 계약번호를 잘못 적어도 아무 신호가 없었다.
- `_findContractIdentityConflicts()` 신규: `get_latest_record_by_contract()` 로
  기존 선사/선명을 조회해 입력값과 대조 (기존 API 재사용, 신규 API 없음)
- 게이트는 `_validateContractNumbersForSave()` 한 곳에 넣었다.
  주간/야간/자동저장이 모두 이 함수를 지나므로 한 번만 막으면 된다.
- **자동저장(alertMode='toast')에서는 confirm() 금지** — 입력 중에 모달이 뜨면
  작업이 끊긴다. 대신 저장을 막고 토스트로 수동 저장을 유도 (기존 충돌감지와 같은 패턴)
- 입력 시점 경고는 자동완성이 이미 받아 온 result 를 재사용 → 추가 조회 없음
- 빈 값·신규 계약번호·계약번호 미입력은 경고하지 않는다 (빈 행에서 잔소리 방지)
