...
def open_erp_input_window(dates_json: str, user_id: str = '') -> Dict[str, Any]:
    """ERP 입력 팝업 창 열기. tkinter 불가 시 인라인 모드로 폴백."""
    try:
        import json as _json
        err = _check_erp_permission(user_id)
        if err:
        ...
        from ..utils.erp_dock_window import ERPDockWindow, _TK_AVAILABLE

        if _TK_AVAILABLE:
            launch_err = ERPDockWindow(dates_records).launch()
            if launch_err:
                return {'success': False, 'message': launch_err}
            return {'success': True, 'message': 'ERP 입력 창을 열었습니다.', 'popup_mode': True}

        # tkinter 없음 → 인라인 모드 폴백
        from ..utils.erp_macro import erp_macro as _macro
        import threading as _threading
        if _macro.get_status()['running']:
            return {'success': False, 'message': '이미 실행 중입니다.'}
        _macro._stop_flag = False
        _threading.Thread(target=_macro.run, args=(dates_records,), daemon=True).start()
        return {'success': True, 'message': 'ERP 자동입력 시작 (팝업 없이 실행 중)', 'popup_mode': False}
