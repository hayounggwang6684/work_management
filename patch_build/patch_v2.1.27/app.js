...
async function openErpInputWindow() {
    ...
    try {
        ...
        if (r.popup_mode === false) {
            // tkinter 불가 — 인라인 진행 UI 표시
            showToast(r.message, 'success');
            document.getElementById('erpInlineProgress')?.classList.remove('hidden');
            document.getElementById('btnErpStart')?.classList.add('hidden');
            document.getElementById('erpProgressFill').style.width = '0%';
            document.getElementById('erpProgressText').textContent = '0 / 0';
            document.getElementById('erpLog').textContent = '';
            if (_erpPollInterval) clearInterval(_erpPollInterval);
...
async function pollErpStatus() {
    try {
        ...
        if (!res.running) {
            ...
            document.getElementById('erpInlineProgress')?.classList.add('hidden');
            // 오류/완료/종료 구분
