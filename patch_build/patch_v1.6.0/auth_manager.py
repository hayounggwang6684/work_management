...
class AuthManager:
    ...
    def create_remember_token(self, user_id: str) -> Optional[str]:
        ...
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    'INSERT INTO auth_sessions (user_id, token, created_at, expires_at, is_active) '
                    'VALUES (?, ?, ?, ?, 1)',
    ...
    def validate_remember_token(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    SELECT s.user_id, u.full_name, u.role, u.status
                ...
                if status not in ('active', 'approved'):
                    return None
                logger.info(f"자동 로그인 토큰 검증 성공: {user_id}")
                return {'user_id': user_id, 'full_name': full_name, 'role': role}
        except Exception as e:
    ...
    def clear_remember_token(self, token: str) -> bool:
        ...
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute('UPDATE auth_sessions SET is_active = 0 WHERE token = ?', (token,))
            return True
        except Exception as e:
