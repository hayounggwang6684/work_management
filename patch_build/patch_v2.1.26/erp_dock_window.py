...
# ---------------------------------------------------------------------------
# embedded Python용 tkinter 경로 보정
# ---------------------------------------------------------------------------
def _patch_tkinter_path() -> str:
    """
    embedded Python은 Lib/ 디렉토리가 sys.path에 없어서 tkinter import가 실패.
    여러 경로를 시도하여 해결. 반환: 추가된 경로 문자열 (없으면 '')
    """
    candidates = [
        # 1순위: sys.executable 기준 Lib/
        pathlib.Path(sys.executable).parent / 'Lib',
        # 2순위: 설치 앱 WorkingDir({app}) 기준 python/Lib/
        pathlib.Path(os.getcwd()) / 'python' / 'Lib',
        # 3순위: 설치 앱 WorkingDir 상위 기준
        pathlib.Path(os.getcwd()).parent / 'python' / 'Lib',
    ]
    for lib_dir in candidates:
        ...
        if tkinter_dir.exists():
            ...
            if lib_str not in sys.path:
                ...
                logger.info(f"tkinter path added: {lib_str}")
            return lib_str
    logger.warning(f"tkinter Lib/ not found. exe={sys.executable}, cwd={os.getcwd()}")
    return ''
...
try:
    ...
    logger.warning(f"tkinter import failed: {_e} | sys.path={sys.path[:5]}")
...
class ERPDockWindow:
    ...
    def launch(self) -> str:
        """
        별도 스레드에서 tkinter 창 실행.
        반환: '' = 성공(스레드 시작됨), 오류 메시지 문자열 = 실패
        """
        if not _TK_AVAILABLE:
            msg = (f"tkinter 초기화 실패: {_TK_IMPORT_ERROR} "
                   f"| exe={sys.executable} | cwd={os.getcwd()}")
            logger.error(msg)
            return msg

        # 스레드 시작 후 0.8초 대기 → tk.Tk() 초기화 실패 여부 감지
        self._start_error: str = ''
        t = threading.Thread(target=self._run_tk, daemon=True)
        t.start()
        t.join(timeout=0.8)  # 최대 0.8초 대기

        if self._start_error:
            return self._start_error
        return ''  # 스레드 살아있으면 정상 시작
    ...
    def _run_tk(self):
        try:
            logger.info("ERPDockWindow: tk.Tk() 시작")
            self.root = tk.Tk()
            logger.info("ERPDockWindow: Tk 인스턴스 생성 완료")
            ...
            self._build_ui()
            logger.info("ERPDockWindow: UI 빌드 완료")

            self.root.after(self.POLL_MS, self._magnet_check)
            self.root.after(self.STATUS_MS, self._poll_status)
            # 500ms 후 최상위로 강제 표시 (Eel 창이 포커스 훔치는 타이밍 이후)
            self.root.after(500, self._force_show)
            self.root.protocol("WM_DELETE_WINDOW", self._on_close)
            logger.info("ERPDockWindow: mainloop 시작")
            self.root.mainloop()
        except Exception as e:
            self._start_error = str(e)
            logger.error(f"ERPDockWindow 실행 오류: {e}", exc_info=True)

    def _force_show(self):
        """창을 화면 앞으로 강제로 가져옴 (Win32 + tkinter 이중 적용)."""
        if not self.root:
            return
        try:
            ...
            # Win32 SetForegroundWindow로 강제 포커스
            ...
            # topmost는 3초 후 해제
            if self.root:
                self.root.after(3000, lambda: self.root.attributes('-topmost', False) if self.root else None)
