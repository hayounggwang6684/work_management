...
def import_excel_data(base64_data: str, username: str = 'admin') -> Dict[str, Any]:
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=1, values_only=False):
            ...
            # 날짜 헤더 행 감지: "날짜 : 2026.02.25." 형식
            date_match = re.search(r'날짜\s*[:\：]\s*(\d{4})\.(\d{2})\.(\d{2})', row_text)
            if date_match:
            ...
            contract_number = str(row[1].value or '').strip()
            company         = str(row[2].value or '').strip() if len(row) > 2 else ''
            ship_name       = str(row[3].value or '').strip().upper() if len(row) > 3 else ''
            engine_model    = str(row[4].value or '').strip().upper() if len(row) > 4 else ''
            work_content    = str(row[5].value or '').strip() if len(row) > 5 else ''
            leader          = get_cell_value(row[6], wrap_italic=True) if len(row) > 6 else ''
            # H열(인덱스 7): 인원 수 — 자동 계산으로 대체, 건너뜀
            teammates       = str(row[8].value or '').strip() if len(row) > 8 else ''
