...
class ERPMacro:
    ...
    def _navigate_to_date(self, date_str: str, hwnd: int, pyautogui,
        ...
        if cal_hwnd:
            ...
            self._log(f"  달력 컨트롤 없음 — 날짜 이동 건너뜀 ({date_str})")
    ...
    def _enter_record(self, record: dict, pyautogui, hwnd: int, win32gui):
        ...
        # 1) ERP 창 포커스 확인 (크롬 팝업에 빼앗겼을 경우 재획득)
        ...
        # 3) 공사(계약번호) 필드 — 계약번호 입력 후 팝업 선택
        if contract:
            # 포커스된 컨트롤에 WM_SETTEXT 시도
            focused = self._get_focused_hwnd(hwnd)
            if focused and not self._set_text_to_control(focused, contract):
                # WM_SETTEXT 실패 시 typewrite 폴백 (ASCII 한정)
        ...
        if work_content:
            focused = self._get_focused_hwnd(hwnd)
            if not focused or not self._set_text_to_control(focused, work_content):
        ...
        if manpower_str and manpower_str != '0':
            focused = self._get_focused_hwnd(hwnd)
            if not focused or not self._set_text_to_control(focused, manpower_str):
        ...
        if workers_str:
            focused = self._get_focused_hwnd(hwnd)
            if not focused or not self._set_text_to_control(focused, workers_str):
    ...
    def _activate_window(self, hwnd, win32gui, win32con):
        ...
        AttachThreadInput으로 현재 포그라운드 스레드와 대상 스레드를 연결하면
        포그라운드 잠금 없이 강제 전환 가능.
        """
        try:
            user32 = ctypes.windll.user32
            ...
            # 현재 포그라운드 창 및 대상 창의 스레드 ID 획득
            fg_hwnd    = win32gui.GetForegroundWindow()
            fg_tid     = user32.GetWindowThreadProcessId(fg_hwnd, None)
            target_tid = user32.GetWindowThreadProcessId(hwnd, None)

            # AttachThreadInput: 두 스레드의 입력 큐를 연결 → 포그라운드 잠금 우회
            attached = False
            if fg_tid and target_tid and fg_tid != target_tid:
                user32.AttachThreadInput(fg_tid, target_tid, True)
                attached = True
