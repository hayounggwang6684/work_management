...
(function () {
        ...
        setTimeout(() => {
            ...
            if (splash) {
                ...
                setTimeout(() => {
                    ...
                    // 자동 로그인 확인 후 로그인 화면 또는 메인 화면 표시
                    // (Eel 준비 완료 후 이 시점에서 호출 → eel 함수 정상 동작 보장)
    ...
    // =========================================================
    // 스플래시 버전 텍스트 최대한 일찍 표시 (eel 준비되는 즉시)
    // =========================================================
    function tryLoadSplashVersion() {
        if (typeof eel === 'undefined' || typeof eel.get_app_info !== 'function') {
            setTimeout(tryLoadSplashVersion, 300);
            return;
        }
        try {
            eel.get_app_info()(function(info) {
                if (info && info.version) {
                    const splashVer = document.getElementById('splashVersionText');
                    if (splashVer) {
                        splashVer.textContent = 'v' + info.version + '\u00A0|\u00A0HA Engineering';
                    }
                }
            });
        } catch(e) {}
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        stepIndex = 1;
        startAutoProgress();
        tryLoadSplashVersion();  // 버전 텍스트 최대한 일찍 표시
