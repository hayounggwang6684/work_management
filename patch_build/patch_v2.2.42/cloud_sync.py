...
class CloudSync:
    ...
    def sync_to_cloud(self) -> bool:
        ...
        try:
            ...
            # 빈 DB 가드: 로컬이 비어있고 클라우드에 데이터가 있으면 업로드 차단
            local_count = self._get_db_record_count(self.local_db_path)
            cloud_count = self._get_db_record_count(cloud_db_path) if cloud_db_path.exists() else 0
            if local_count == 0 and cloud_count > 0:
                logger.warning(
                    f"업로드 차단: 로컬 DB가 비어있음 (cloud={cloud_count}건) — "
                    "클라우드 데이터 보호를 위해 업로드를 건너뜁니다."
                )
                return False
    ...
    def sync_from_cloud(self) -> bool:
        ...
        try:
            ...
            # 빈 클라우드 가드: 클라우드 DB가 비어있으면 pull 차단
            cloud_count = self._get_db_record_count(cloud_db_path)
            if cloud_count == 0:
                logger.warning("클라우드 DB가 비어있음 — pull을 건너뜁니다.")
                return False
    ...
    def _get_db_record_count(self, db_path) -> int:
        """DB의 work_records 레코드 수 반환 (실패 시 -1)"""
        try:
            import sqlite3 as _sqlite3
            conn = _sqlite3.connect(str(db_path), timeout=5)
            cursor = conn.cursor()
    ...
    def connect_external(self, cloud_path: str) -> dict:
        ...
        """
        try:
            # 1. 경로 검증 (트래버설 방지: resolve로 실제 경로 확인)
            cloud_dir = Path(cloud_path).resolve()
            ...
            if lock_path.exists():
                try:
                    ...
                    logger.warning(f"잠금 파일 파싱 실패: {e}")
                    warning = '다른 PC에서 사용 중일 수 있습니다.'

            # 4. 로컬 DB 백업 (롤백용 경로 보관)
            ...
            if self.local_db_path.exists():
                ...
                shutil.copy2(self.local_db_path, local_backup_path)
                logger.info(f"외부 연결 전 로컬 백업: {local_backup_path}")

            # 5. 클라우드 DB → 로컬 복사 + 무결성 검증
            self.local_db_path.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(cloud_db, self.local_db_path)
            try:
                import sqlite3 as _sqlite3
                _conn = _sqlite3.connect(str(self.local_db_path))
                _conn.execute('PRAGMA integrity_check')
                _conn.close()
            except Exception as e:
                logger.error(f"클라우드 DB 무결성 검증 실패: {e}")
                if local_backup_path and local_backup_path.exists():
                    shutil.copy2(local_backup_path, self.local_db_path)
                    logger.info("로컬 백업으로 롤백 완료")
                return {'success': False, 'message': '클라우드 DB 파일이 손상되었습니다. 이전 상태로 복원했습니다.'}
