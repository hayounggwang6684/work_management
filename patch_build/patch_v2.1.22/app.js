...
async function openErpInputWindow() {
    ...
    const startDate = document.getElementById('erpStartDate')?.value;
    const endDate   = document.getElementById('erpEndDate')?.value;
    console.log('[ERP] openErpInputWindow 호출, checked:', checks.length, 'start:', startDate, 'end:', endDate);
    try {
        const res = await eel.get_records_for_erp(startDate, endDate, currentUser?.user_id || '')();
        console.log('[ERP] get_records_for_erp:', res?.success, res?.message);
        ...
        console.log('[ERP] filtered 건수:', filtered.length);
        ...
        console.log('[ERP] open_erp_input_window:', r?.success, r?.message);
        if (r.success) {
            showToast('ERP 입력 창이 열립니다. 잠시 후 새 창을 확인하세요.', 'success');
        } else {
            showToast(r.message || '팝업 창 실행 실패', 'error');
        }
    } catch (e) {
        console.error('[ERP] 오류:', e);
