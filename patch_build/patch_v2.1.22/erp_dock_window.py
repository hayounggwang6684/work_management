...
class ERPDockWindow:
    ...
    def _run_tk(self):
        try:
            ...
            # 창을 최상위로 강제 표시 (Eel Chrome 창 뒤에 가리지 않도록)
            ...
            # topmost는 2초 후 해제 (영구 최상위 고정은 불편함)
            self.root.after(2000, lambda: self.root.attributes('-topmost', False) if self.root else None)
